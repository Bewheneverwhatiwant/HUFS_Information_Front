      ...
      children: [
        ...
        children: [
          ...
          onPressed: isButtonCEnabled ? () {
            setState(() {
              isButtonCEnabled = false; //false, true 바꿔서 '나가기' 버튼 작동 여부(isDone==icons.length일 때 활성화되는지) 테스트 가능
        ...
        backgroundColor: MaterialStateProperty.resolveWith<Color>((states) {return Colors.white;},
    ),
  ),
        )
        ),
          ...
          Container(
            ...
            child: ElevatedButton(
              onPressed: isButtonAEnabled ? () {
                setState(() {
                  isButtonAEnabled = false; //'참가확정' 버튼 클릭 시 '참가확정' 버튼을 비활성화하여 한번만 클릭 가능하도록 함
                  isButtonBEnabled = false; //'참가확정' 버튼 클릭 시 '나가기' 버튼 비활성화
                  isButtonCEnabled = true; //'참가확정' 버튼 클릭 시 '함께 약속한 내용 완료!' 버튼이 활성화
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: () {
                ...
                if (isButtonAEnabled) {
                  showSnackbar(context, '채팅방 참가가 취소되었습니다!'); //참가를 확정하지 않았다면, 나가기 가능!
                  Navigator.pushAndRemoveUntil(context, MaterialPageRoute(
                  builder: (BuildContext context) =>
                    MainPage(context: context, isButtonEnabled: isButtonEnabled,)), (route) => false); 
                    //나가기 버튼 클릭 시 홈화면으로 이동
                }

                //'참가확정' 버튼을 눌러서 '나가기' 버튼이 비활성화된 상태에서 '나가기' 버튼을 누르면? (isButtonAEnabled = false 일 때)
                else if (isDone < icons.length) {
                  showSnackbar(context, '아직 모든 참가자가 모집 종료에 동의하지 않았습니다!');
                }
